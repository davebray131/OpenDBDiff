...
{
    private static readonly Regex ExtractBuildPathRegex = new($@"\s+at OpenDBDiff.Front.{nameof(ErrorForm)}.{nameof(GetBuildPath)}\(\) in (.*\\)OpenDBDiff\\Front\\ErrorForm.cs", RegexOptions.Compiled);
    private static readonly Regex SystemExceptionsRegex = new(@"\s+at System\.[^\r\n]+\r\n", RegexOptions.Compiled);
    ...
    public void LoadException(Exception ex)
    {
        ...
        var queryMaxLength = 280; //Bug in Github for searching issues? Q max length is 280
