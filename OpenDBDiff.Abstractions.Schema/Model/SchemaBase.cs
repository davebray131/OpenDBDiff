...
public abstract class SchemaBase(string nameCharacterOpen, string nameCharacterClose, ObjectType objectType) : ISchemaBase
{
    private ObjectStatus status = ObjectStatus.Original;
    private ISchemaBase parent;
    private readonly string nameCharacterOpen = nameCharacterOpen;
    private readonly string nameCharacterClose = nameCharacterClose;
    ...
    public IDatabase RootParent
    {
        get
        {
            ...
            if (Parent != null)
            {
                rootParent = Parent.Parent != null
                    ? Parent.Parent.Parent != null ? (IDatabase)Parent.Parent.Parent : (IDatabase)Parent.Parent
                    : (IDatabase)Parent;
            }
    ...
    public int CompareFullNameTo(string name, string myName) => !RootParent.IsCaseSensitive ? myName.ToUpper().CompareTo(name.ToUpper()) : myName.CompareTo(name);
    ...
    /// <returns></returns>
    public virtual ISchemaBase Clone(ISchemaBase parent) => null;
    ...
    /// </returns>
    public virtual SQLScriptList ToSqlDiff(ICollection<ISchemaBase> schemas) => null;

    public virtual SQLScript Create() => throw new NotImplementedException();
    ...
    public bool GetWasInsertInDiffList(ScriptAction action) => wasInsertInDiffList != null && wasInsertInDiffList.ContainsKey(action);
    ...
    public void ResetWasInsertInDiffList() => wasInsertInDiffList = null;
    ...
    /// </summary>
    public virtual string FullName => string.IsNullOrEmpty(Owner)
                ? nameCharacterOpen + Name + nameCharacterClose
                : nameCharacterOpen + Owner + nameCharacterClose + "." + nameCharacterOpen + Name + nameCharacterClose;
    ...
    public bool HasState(ObjectStatus statusFind) => (Status & statusFind) == statusFind;

    public virtual bool IsCodeType => false;

    public virtual int DependenciesCount => 0;

    public virtual bool HasDependencies => DependenciesCount > 0;
    ...
    /// </summary>
    public virtual bool MustBuildSqlInLine => false;
}
