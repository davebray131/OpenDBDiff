...
internal static class ConstraintSQLCommand
{
    public static string GetUniqueKey(DatabaseInfo.SQLServerVersion version, DatabaseInfo.SQLServerEdition edition)
    {
        switch (version)
        {
            ...
            default:
                return edition == DatabaseInfo.SQLServerEdition.Azure ? GetUniqueKeyAzure() : GetUniqueKey2008();
        }
    }
    ...
    private static string GetUniqueKeyAzure()
    {
        ...
        _ = sql.Append("SELECT O.type as ObjectType, S.Name as Owner, I.object_Id AS id,'' as FileGroup, C.user_type_id, C.column_id, I.Index_id, C.Name AS ColumnName, I.Name, I.type, I.fill_factor, I.is_padded, I.allow_row_locks, I.allow_page_locks, I.ignore_dup_key, I.is_disabled, IC.is_descending_key, IC.is_included_column ");
        _ = sql.Append("FROM sys.indexes I ");
        _ = sql.Append("INNER JOIN sys.objects O ON O.object_id = I.object_id ");
        _ = sql.Append("INNER JOIN sys.schemas S ON S.schema_id = O.schema_id ");
        _ = sql.Append("INNER JOIN sys.index_columns IC ON IC.index_id = I.index_id AND IC.object_id = I.object_id ");
        _ = sql.Append("INNER JOIN sys.columns C ON C.column_id = IC.column_id AND IC.object_id = C.object_id ");
        //sql.Append("LEFT JOIN sys.data_spaces AS dsidx ON dsidx.data_space_id = I.data_space_id ");
        _ = sql.Append("WHERE is_unique_constraint = 1 AND O.type <> 'TF' ORDER BY I.object_id,I.Name");
        return sql.ToString();
    }
    ...
    private static string GetPrimaryKeyAzure()
    {
        ...
        _ = sql.Append("INNER JOIN sys.columns C ON C.column_id = IC.column_id AND IC.object_id = C.object_id ");
        //sql.Append("LEFT JOIN sys.data_spaces AS dsidx ON dsidx.data_space_id = I.data_space_id ");
        _ = sql.Append("WHERE is_primary_key = 1 AND O.type <> 'TF' ORDER BY I.object_id, IC.key_ordinal");
        return sql.ToString();
    }
    ...
    private static string GetPrimaryKey2005()
    {
        ...
        _ = sql.Append("INNER JOIN sys.index_columns IC ON IC.index_id = I.index_id AND IC.object_id = I.object_id ");
        _ = sql.Append("INNER JOIN sys.columns C ON C.column_id = IC.column_id AND IC.object_id = C.object_id ");
        _ = sql.Append("INNER JOIN sys.data_spaces AS dsidx ON dsidx.data_space_id = I.data_space_id ");
        _ = sql.Append("WHERE is_primary_key = 1 AND O.type <> 'TF' ORDER BY I.object_id, IC.key_ordinal");
        return sql.ToString();
    }
