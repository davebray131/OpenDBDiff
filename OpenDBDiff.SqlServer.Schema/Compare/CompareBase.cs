...
internal abstract class CompareBase<T> where T : ISchemaBase
{
    ...
    protected void DoDelete(T node) => node.Status = ObjectStatus.Drop;
    ...
    public void GenerateDifferences<Root>(SchemaList<T, Root> originFields, SchemaList<T, Root> destinationFields) where Root : ISchemaBase
    {
        ...
        var originIndex = 0;
        var destinationCount = destinationFields.Count;
        var originCount = originFields.Count;
        T node;
    ...
    {
        var dropList = (from node in origin.ExtendedProperties
        ...
        var addList = (from node in destination.ExtendedProperties
