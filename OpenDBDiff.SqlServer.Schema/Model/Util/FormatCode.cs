...
internal static class FormatCode
{
    ...
    public static FindCreateStatementResult FindCreate(string body)
    {
        ...
        var entityNameToken = token1.AsCharacter?.Text == "." ? token2.AsIdentifier : token0.AsIdentifier;
    ...
    public static string FormatAlter(string objectType, string body, ISchemaBase item, bool quitSchemaBinding)
    {
        ...
        try
        {
            ...
            var sitem = FindAndNormalizeCreate(item, prevText);
            ...
            {
                var text = RegCreateAlter.Replace(sitem.Body, "ALTER", 1, sitem.FindPosition);
    ...
    public static string FormatCreate(string objectType, string body, ISchemaBase item)
    {
        try
        {
            var prevText = (string)body.Clone();
    ...
    /// </summary>
    private static string CleanLast(string body) => string.IsNullOrEmpty(body) ? string.Empty : body.TrimStart().TrimEnd(TrimCharacters);
